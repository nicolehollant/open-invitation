<template>
<div>
  <header class="block w-full flex items-center justify-between">
    ...
    <h2 class="hidden serif sm:block text-4xl font-semibold text-blue-600">
      {{ title }}
    </h2>
    <div class="relative" v-on-click-outside="close">
      <div class="w-32 flex justify-end">
        <Icon 
          ...
          icon="menu" 
          size="8" 
          class="text-blue-700 rounded-lg focus:bg-yellow-200 focus:outline-none focus:shadow-outline" 
          tabindex="0"
        />
      </div>
      <nav class="absolute z-50 right-0 p-1 mr-1 w-max flex flex-col space-y-4 align-start bg-yellow-100 rounded-md shadow-xl border border-blue-400" v-if="menuUp">
        <router-link 
          v-for="link of links" 
          :key="link.to" 
          :to="link.to" 
  ...
  <h2 class="serif sm:hidden text-3xl font-semibold text-blue-600 text-center">
    {{ title }}
  </h2>
</div>
</template>
